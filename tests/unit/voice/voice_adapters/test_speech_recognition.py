def test_speechrecognition_mic_id(config):
    ...
    with patch("vinisto.services.voice.config", return_value=config) as cmock:
        import vinisto.services.voice as sr
        with patch("vinisto.services.voice.speech") as mock:
